...
export function activate(context: vscode.ExtensionContext) {
	...
	console.log('Congratulations, your extension "diagrammaticv3" is now active!');
...
class CatCodingPanel {
	...
	private _getHtmlForWebview(webview: vscode.Webview, catGifPath: string) {
		...
		const scriptPathOnDisk = vscode.Uri.joinPath(this._extensionUri, viewPath, 'index.js');

		// And the uri we use to load this script in the webview
		const scriptUri = webview.asWebviewUri(scriptPathOnDisk);

		// Local path to css styles
		const stylesPathMainPath = vscode.Uri.joinPath(this._extensionUri, 'src', 'views', 'vscode.css');
